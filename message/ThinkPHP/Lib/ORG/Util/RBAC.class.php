...
class RBAC extends Think
{
    ...
    {
        //检查是否需要认证
       // echo $_SESSION[$accessGuid];
        if(RBAC::checkAccess()) {
            //存在认证识别号，则进行进一步的访问决策
            $accessGuid   =   md5($appName.MODULE_NAME.ACTION_NAME);   
            //echo $appName."||".$module."||".ACTION_NAME;     
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
              ...
              // echo $appName."||".$module."||".ACTION_NAME;
              // if(ACTION_NAME=='newmenu') $appName=APP_NAME;
    ...
    static public function getAccessList($authId)
    {
        ...
        $sql    =   "select node.id,node.name from ".
                    ...
                    "and role.status=1 and access.node_id=node.id and node.level=1 and node.status=1 ".
                    "order by node.sort";
        $apps =   $db->query($sql);
       // dump($apps);
        $access =  array();
        foreach($apps as $key=>$app) {
            ...
            $sql    =   "select node.id,node.name from ".
                    ...
                    "where user.user_id='{$authId}' and user.role_id=role.id ".
                    "and (access.role_id=role.id or (access.role_id=role.pid and role.pid!=0 )) ".
                    "and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status=1 ".
                    "order by node.sort";
            ...
            foreach($modules as $key=>$module) {
                ...
                $sql    =   "select node.id,node.name from ".
                    ...
                    "where user.user_id='{$authId}' and user.role_id=role.id ".
                    "and (access.role_id=role.id or (access.role_id=role.pid and role.pid!=0)) ".
                    "and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId} and node.status=1 ".
                    "order by node.sort";
                    $rs = $db->query($sql);
                    foreach ($rs as $a){
        ...
        $table = array('role'=>C('RBAC_ROLE_TABLE'),'user'=>C('RBAC_USER_TABLE'),'access'=>'mj_menu_access','node'=>'mj_admin_menu');
        $sql    =   "select menu.menu_code,menu.menu_name from ".
                    $table['role']." as role,".
                    $table['user']." as user,".
                    $table['access']." as access ,".
                    $table['node']." as menu ".
                    "where user.user_id='{$authId}' and user.role_id=role.id ".
                    "and (access.role_id=role.id or (access.role_id=role.pid and role.pid!=0)) ".
                    "and role.status=1 and access.node_id=menu.menu_code  ".
                    "order by menu.menu_code";
        $menu =   $db->query($sql);
        $rsmenu = array();
        foreach ($menu as $node){
            $rsmenu[]	=	$node['menu_code'];
        }        
        $access['menu']= $rsmenu;    
        return $access;
    }
